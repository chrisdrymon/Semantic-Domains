# Given a lemma, its part of speech, and the lemma and part of speech of its dependent's dependent, this will return
# the PMIs of the dependant's semantic domains. It's especially useful for investigating how the presence of the article
# effects PMIs.
...
import pandas as pd
from utility import deaccent, poser, give_dependents, semdom_poser
                    ...
                    for f_article in give_dependents(sentence_words, f_dependent):
                        if f_article.has_attr('lemma'):
                            if deaccent(f_article['lemma']).lower() == dependent_dependent_lemma\
                                    and poser(f_article) == dependent_dependent_pos:
